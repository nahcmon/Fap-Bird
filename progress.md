...
- Desktop single-player no longer uses the old translucent gradient “lens” around the lane; the backdrop outside the active render box is now a hard black matte with a crisp black edge, while mobile keeps the fullscreen sky/ground treatment.
- Follow-up browser verification sampled desktop pixels on both sides of the active lane as pure black (`0,0,0,255`) and confirmed phone-sized single-player still reports `layout: "mobile-fullscreen"` with the mobile-only menu.
